...
const localStorageDataService: IDataService = {
  async initializeData(): Promise<void> {
    if (typeof window === 'undefined' || isDataInitialized) {
      // console.log("Local data already initialized or not in browser.");
      return;
    }
    ...
    // console.log("Local Storage Data Initialized.");
  },
  ...
  async deleteProduct(productId): Promise<boolean> {
    ...
    if (products.length < initialLength) {
      ...
      if (imageIdsToDelete.length > 0) {
        await this.deleteImagesForEntity(imageIdsToDelete);
      }
  ...
  async findProductById(productId): Promise<Product | undefined> {
    const products = await this.getProducts();
  ...
  async deleteCategory(categoryId): Promise<boolean> {
    ...
    if (categories.length < initialLength) {
      ...
      for (const child of childCategories) {
          child.parentId = null;
          await this.updateCategory(child);
      }
      return true;
    }
    return false;
  },
  async findCategoryById(categoryId): Promise<Category | undefined> {
    if (!categoryId) return undefined;
    const categories = await this.getCategories();
    return categories.find(c => c.id === categoryId);
  },
  async getChildCategories(parentId): Promise<Category[]> {
    const categories = await this.getCategories();
  ...
  async getCart(userId): Promise<Cart | null> {
      ...
      if (userCart) {
        userCart.savedForLaterItems = userCart.savedForLaterItems || [];
        return userCart;
      }
  ...
  async updateCart(cart): Promise<void> {
      ...
      const updatedSavedForLaterItems: CartItem[] = [];
       for (const item of (cart.savedForLaterItems || [])) {
        const product = await this.findProductById(item.productId);
        updatedSavedForLaterItems.push({
            ...item,
            name: product?.name || item.name,
            primaryImageId: product?.primaryImageId || item.primaryImageId,
        });
  ...
  async clearCart(userId): Promise<void> {
      const cart = await this.getCart(userId);
      if (cart) {
          await this.updateCart({ ...cart, items: [], updatedAt: new Date().toISOString() });
      }
  },
  ...
  async getWishlist(userId): Promise<WishlistItem[]> {
    const all = getItem<WishlistItem[]>(KEYS.WISHLISTS) || [];
  ...
  async removeFromWishlist(userId, productId): Promise<void> {
    let wishlists = getItem<WishlistItem[]>(KEYS.WISHLISTS) || [];
    wishlists = wishlists.filter(item => !(item.userId === userId && item.productId === productId));
    setItem(KEYS.WISHLISTS, wishlists);
  },
  async isProductInWishlist(userId, productId): Promise<boolean> {
    const all = getItem<WishlistItem[]>(KEYS.WISHLISTS) || [];
    return all.some(item => item.userId === userId && item.productId === productId);
  },

  async getReviewsForProduct(productId): Promise<Review[]> {
    const all = getItem<Review[]>(KEYS.REVIEWS) || [];
    return all.filter(review => review.productId === productId).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  },
  ...
  async getRecentlyViewed(userId): Promise<RecentlyViewedItem[]> {
    const allLogs = getItem<UserRecentlyViewed[]>(KEYS.RECENTLY_VIEWED) || [];
    const userLog = allLogs.find(log => log.userId === userId);
  ...
  async saveImage(entityId, imageType, imageFile): Promise<string> {
    return saveImageToDB(entityId, imageType, imageFile);
  },
  async getImage(imageId): Promise<Blob | null> {
    return getImageFromDB(imageId);
  },
  async deleteImage(imageId): Promise<void> {
    return deleteImageFromDB(imageId);
  },
  async deleteImagesForEntity(imageIds): Promise<void> {
    return deleteImagesForEntityFromDB(imageIds);
  }
};
