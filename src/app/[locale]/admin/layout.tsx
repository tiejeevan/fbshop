...
// Link from next-intl/link for locale-aware navigation if needed, or ensure paths are correct
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleLogout = () => {
    logout();
    router.push('/'); // Redirect to homepage, middleware will handle locale
  };

  React.useEffect(() => {
    if (!isLoading) {
      if (!currentUser || currentUser.role !== 'admin') {
        router.replace('/products'); // Redirect to customer products page
      }
    }
      ...
      <div className="flex flex-col flex-1">
        ...
        <SidebarInset>
          <div className="p-4 md:p-6 overflow-auto">
             {/* Key is pathname to re-trigger animation on route change */}
